...
from legged_lab.envs.g1.qie_walk_cfg import G1QieWalkFlatEnvCfg
...
class G1Env(VecEnv):
    def __init__(self, cfg: G1QieWalkFlatEnvCfg, headless, ):
        self.cfg: G1QieWalkFlatEnvCfg
        self.cfg = cfg
    ...
    def init_buffers(self):
        ...
        self.elbow_body_ids, _ = self.robot.find_bodies(
            name_keys=["left_wrist_roll_rubber_hand", "right_wrist_roll_rubber_hand"], preserve_order=True
        )
        ...
        self.left_arm_local_vec = torch.tensor([0.18, 0.0, 0.0], device=self.device).repeat((self.num_envs, 1))
        self.right_arm_local_vec = torch.tensor([0.18, 0.0, 0.0], device=self.device).repeat((self.num_envs, 1))
    ...
    def visualize_motion(self, time):
        ...
        dof_pos[:, self.left_arm_ids] = visual_motion_frame[19:24]
        dof_pos[:, self.right_arm_ids] = visual_motion_frame[24:29]

        dof_vel[:, self.left_leg_ids] = visual_motion_frame[35:41]
        dof_vel[:, self.right_leg_ids] = visual_motion_frame[41:47]
        dof_vel[:, self.waist_ids] = visual_motion_frame[47:48]
        dof_vel[:, self.left_arm_ids] = visual_motion_frame[48:53]
        dof_vel[:, self.right_arm_ids] = visual_motion_frame[53:58]
        ...
        lin_vel = visual_motion_frame[29:32].clone()
        ang_vel = visual_motion_frame[32:35].clone()
    ...
    def compute_current_observations(self):
        ...
        current_critic_obs = torch.cat([current_actor_obs, root_lin_vel * self.obs_scales.lin_vel, feet_contact],
                                       dim=-1)
    ...
    def init_obs_buffer(self):
        if self.add_noise:
            ...
            noise_vec[12 + self.num_actions * 3: 16 + self.num_actions * 3] = 0.0
    ...
    def update_terrain_levels(self, env_ids):
        ...
        move_down = (
                distance < torch.norm(self.command_generator.command[env_ids, :2],
                                      dim=1) * self.max_episode_length_s * 0.5
        )
        move_down *= ~move_up
