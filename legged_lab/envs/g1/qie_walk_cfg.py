...
from legged_lab.assets.g1 import UNITREE_G1_23DOF_CFG
...
class LiteRewardCfg:
    ...
    ang_vel_y_l2 = RewTerm(func=mdp.ang_vel_y_l2, weight=-0.1)
    ang_vel_x_l2 = RewTerm(func=mdp.ang_vel_x_l2, weight=-0.01)
    ...
    undesired_contacts = RewTerm(
        ...
        params={
            "sensor_cfg": SceneEntityCfg(
                "contact_sensor", body_names=[".*knee.*", ".*shoulder.*", ".*elbow.*", ".*wrist.*","pelvis","torso.*"]
            ),
            "threshold": 1.0,
        },
    )
    flat_orientation_x = RewTerm(func=mdp.flat_orientation_x, weight=-6.0)
    flat_orientation_y = RewTerm(func=mdp.flat_orientation_y, weight=-0.6)
    ...
    hip_pitch_action = RewTerm(func=mdp.hip_pitch_action, weight=-0.01)
...
class G1QieWalkFlatEnvCfg:
    ...
    robot: RobotCfg = RobotCfg(
        ...
        action_scale=0.25,
        terminate_contacts_body_names=[".*knee.*", ".*shoulder.*", ".*elbow.*", ".*wrist.*","pelvis","torso.*"],
...
@configclass
class G1QieWalkAgentCfg(RslRlOnPolicyRunnerCfg):
    seed = 42
    device = "cuda:0"
    ...
    experiment_name = "g1qiewalk"
    run_name = ""
    logger = "tensorboard"
    neptune_project = "g1qiewalk"
    wandb_project = "g1qiewalk"
    resume = False
    load_run = ".*"
