...
class AMPLoaderDisplay:
    # Each motion frame is two equal halves: a pose half [root_pos(3), root_rot(3), joint_pos(J)]
    # and a velocity half [root_lin_vel(3), root_ang_vel(3), joint_vel(J)]. The half width
    # (ROOT_STATES_NUM + J) differs per robot (g1 23dof -> 29, tienkung 20dof -> 26), so it is
    # inferred per-instance from the frame width (see __init__) rather than hard-coded.
    ROOT_STATES_NUM = 6
    ...
    def __init__(
        ...
        # Infer the half-frame width from the first motion file. Each frame = pose half + vel half,
        # each of width (ROOT_STATES_NUM + joint_dim), so the half width is simply frame_width / 2.
        with open(motion_files[0]) as f:
            first_frame_width = np.array(json.load(f)["Frames"]).shape[1]
        half_width = first_frame_width // 2
        self.joint_pose_start_idx = 0
        self.joint_pose_end_idx = half_width
        self.joint_vel_start_idx = half_width
        self.joint_vel_end_idx = 2 * half_width
        ...
        for i, motion_file in enumerate(motion_files):
            ...
            with open(motion_file) as f:
                ...
                self.trajectories.append(
                    torch.tensor(
                        motion_data[:, : self.joint_vel_end_idx], dtype=torch.float32, device=device
                    )
                )
